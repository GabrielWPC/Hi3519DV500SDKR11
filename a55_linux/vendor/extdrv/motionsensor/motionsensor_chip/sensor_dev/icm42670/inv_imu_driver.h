...
typedef struct {
	td_s32      sensor_mask;
	td_u16 timestamp_fsync;
	td_s16  accel[3];
	td_s16  gyro[3];
	td_s16  temperature;
	td_s8   accel_high_res[3];
	td_s8   gyro_high_res[3];
...
struct inv_imu_device {
	 ...
	 */
	void (*sensor_event_cb)(td_u16 packet_count, td_u16 idx, td_u64 time_inter, inv_imu_sensor_event_t *event);

	td_u8               fifo_data[FIFO_MIRRORING_SIZE]; /**< FIFO mirroring memory area */
	td_u8               dmp_is_on; /**< DMP started status */
	td_u8               endianness_data; /**< Data endianness configuration */
	td_u8               fifo_highres_enabled; /**< Highres mode configuration */
	INV_IMU_FIFO_CONFIG_t fifo_is_used; /**< FIFO configuration */
	td_u64              gyro_start_time_us; /**< Gyro start time used to discard first samples */
	td_u64 accel_start_time_us; /**< Accel start time used to discard first samples */
};
 ...
 *  @return       0 on success, negative value on error.
 */
td_s32 inv_imu_enable_accel_low_power_mode(struct inv_imu_device *s);
 ...
 *  @return       0 on success, negative value on error.
 */
td_s32 inv_imu_disable_accel(struct inv_imu_device *s);
 ...
 *  @return       0 on success, negative value on error.
 */
td_s32 inv_imu_disable_gyro(struct inv_imu_device *s);
 ...
 *  @return       0 on success, negative value on error.
 */
td_s32 inv_imu_enable_fsync(struct inv_imu_device *s);
 ...
 *  @return                            0 on success, negative value on error.
 */
td_s32 inv_imu_set_config_int1(struct inv_imu_device *s, inv_imu_interrupt_parameter_t *interrupt_to_configure);
 ...
 *  @return                            0 on success, negative value on error.
 */
td_s32 inv_imu_get_config_int1(struct inv_imu_device *s, inv_imu_interrupt_parameter_t *interrupt_to_configure);
 ...
 *  @return                            0 on success, negative value on error.
 */
td_s32 inv_imu_set_config_int2(struct inv_imu_device *s, inv_imu_interrupt_parameter_t *interrupt_to_configure);
 ...
 *  @return                            0 on success, negative value on error.
 */
td_s32 inv_imu_get_config_int2(struct inv_imu_device *s, inv_imu_interrupt_parameter_t *interrupt_to_configure);
 ...
 *  @return       0 on success, negative value on error.
 */
td_s32 inv_imu_get_data_from_registers(struct inv_imu_device *s);
 ...
 */
td_s32 inv_imu_get_data_from_fifo(struct inv_imu_device *s);
 ...
 *  @return       0 on success, negative value on error.
 */
td_s32 inv_imu_enable_high_resolution_fifo(struct inv_imu_device *s);

/** @brief Disable 20 bits raw acc and raw gyr data in fifo.
 *  @param[in] s  Pointer to device.
 *  @return       0 on success, negative value on error.
 */
td_s32 inv_imu_disable_high_resolution_fifo(struct inv_imu_device *s);
 ...
 */
td_u32 inv_imu_get_timestamp_resolution_us(struct inv_imu_device *s);
 ...
 *  @return                0 on success, negative value on error.
 */
td_s32 inv_imu_set_endianness(struct inv_imu_device *s, INTF_CONFIG0_DATA_ENDIAN_t endianness);
 ...
 *  @return                0 on success, negative value on error.
 */
td_s32 inv_imu_configure_fifo_data_rate(struct inv_imu_device *s, FDR_CONFIG_FDR_SEL_t dec_factor);

/** @brief Return driver version x.y.z-suffix as a td_char array
 *  @return driver version a td_char array "x.y.z-suffix"
 */
const td_char *inv_imu_get_version(void);
