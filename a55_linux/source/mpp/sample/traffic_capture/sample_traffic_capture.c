...
static td_s32 sample_traffic_capture_sys_init(ot_vi_vpss_mode_type mode_type, ot_vi_aiisp_mode aiisp_mode,
    sample_sns_type sns_type)
{
    ...
    sample_vb_param g_vb_param = {
        ...
        .compress_mode = {OT_COMPRESS_MODE_NONE, OT_COMPRESS_MODE_SEG},
...
static td_bool sample_is_capture_frame(ot_video_frame_info *capture_frame_info)
{
    ...
    capture_frame = ((rd % 64) == 0) ? TD_TRUE : TD_FALSE; /* 64: 1/64 probability */
...
static td_s32 sample_traffic_capture_offline(td_void)
{
    ...
    const ot_vi_pipe capture_pipe = 7;
...
td_void sample_traffic_capture_usage(const char *prg_name)
{
...
#endif
{
    ...
    index = (td_u32)strtol(argv[1], &end_ptr, 10); /* base 10, argv[1] has been check between [0, 9] */
