...
static td_void sample_fov_to_ldcv2_print(const sample_fov_attr *fov_attr, ot_dis_ldc_attr *dis_ldc_attr, td_float fov)
{
    printf("\nInput:");
...
static td_void sample_fov_to_ldcv2_assign(const sample_fov_attr *fov_attr, ot_dis_ldc_attr *dis_ldc_attr,
    td_float fov_horizontal, td_float fov_vertical)
{
    const td_s32 dist_coef_prec = 100000;

    dis_ldc_attr->focal_len_x = (td_s32)(round((fov_attr->width / 2) /    /* 2 half */
        tan(fov_horizontal * PI / 180 / 2) * 100));                     /* 180 degree, 2 half, 100 gradient */
    dis_ldc_attr->focal_len_y = (td_s32)(round((fov_attr->height / 2) /   /* 2 half */
        tan(fov_vertical * PI / 180 / 2) * 100));                       /* 180 degree, 2 half, 100 gradient */

    dis_ldc_attr->coord_shift_x = (td_s32)(round(fov_attr->width / 2 * 100)); /* 2 half, 100 gradient */
    dis_ldc_attr->coord_shift_y = (td_s32)(round(fov_attr->height / 2 * 100)); /* 2 half, 100 gradient */
...
td_s32 ot_sample_fov_to_ldcv2(const sample_fov_attr *fov_attr, ot_dis_ldc_attr *dis_ldc_attr)
{
    ...
    td_float fov = (td_float)fov_attr->fov / (td_float)(1 << FOV_PREC_BITS);

    if ((fov_attr == TD_NULL) || (dis_ldc_attr == TD_NULL)) {
        return TD_FAILURE;
    }

    (td_void)memset_s(dis_ldc_attr, sizeof(ot_dis_ldc_attr), 0, sizeof(ot_dis_ldc_attr));
    ...
    switch (fov_attr->type) {
        case OT_FOV_TYPE_DIAGONAL: /* input diagonal field of view angle */
            ...
            fov_vertical   = fov * fov_attr->height /
                sqrt(fov_attr->width * fov_attr->width + fov_attr->height * fov_attr->height);
            break;
        case OT_FOV_TYPE_HORIZONTAL: /* input horizontal field of view angle */
            fov_horizontal = fov;
