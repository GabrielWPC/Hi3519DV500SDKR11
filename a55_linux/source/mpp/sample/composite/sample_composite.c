...
static td_s32 sample_venc_set_gop_mode(td_s32 c, ot_venc_gop_mode *gop_mode)
{
    switch (c) {
        case '0':
...
static td_s32 get_gop_mode(ot_venc_gop_mode *gop_mode)
{
    ...
    while (TD_TRUE) {
        ...
        if (c == ENTER_ASCII) {
            ...
            continue;
        } else if (sample_venc_set_gop_mode(c, gop_mode) == TD_SUCCESS &&
...
static td_s32 sample_set_rc_mode(td_s32 c, sample_rc *rc_mode)
{
    switch (c) {
        case 'c':
...
td_s32 get_rc_mode(ot_payload_type type, sample_rc *rc_mode)
{
    ...
    while (TD_TRUE) {
        ...
        if (c == ENTER_ASCII) {
            sample_print("invalid input! please try again.\n");
            continue;
        } else if (sample_set_rc_mode(c, rc_mode) == TD_SUCCESS &&
            sample_venc_getchar() == ENTER_ASCII) {
            return TD_SUCCESS;
        }
        sample_print("invalid input! please try again.\n");
...
td_s32 sample_composite_sys_init(sample_composite_vb_attr *vb_attr)
{
    ...
    if (vb_attr->valid_num > OT_VB_MAX_COMMON_POOLS) {
        sample_print("sample_composite_sys_init vb valid num(%u) too large than OT_VB_MAX_COMMON_POOLS(%d)!\n",
...
static td_s32 sample_composite_vi_vpss_cfg(sample_composite_param *param)
{
    td_s32 i, ret;
