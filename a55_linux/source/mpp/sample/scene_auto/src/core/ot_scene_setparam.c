...
td_s32 ot_scene_set_dynamic_aibnr_part(ot_vi_pipe vi_pipe, td_u8 index)
{
    td_s32 ret;
    td_u32 i;
    td_bool is_wdr_mode;
...
static td_s32 ot_scene_enable_ai3dnr(ot_vi_pipe vi_pipe)
{
    td_s32 ret;
    ot_ai3dnr_status status;
    ret = ss_mpi_ai3dnr_query_status(vi_pipe, &status);
    if (ret != TD_SUCCESS) {
        return ret;
    }
    if (status.enable == TD_TRUE) {
        return TD_SUCCESS;
    }
    return ss_mpi_ai3dnr_enable(vi_pipe);
}

static td_s32 ot_scene_disable_ai3dnr(ot_vi_pipe vi_pipe)
{
    td_s32 ret;
    ot_ai3dnr_status status;
    ret = ss_mpi_ai3dnr_query_status(vi_pipe, &status);
    ...
    if (status.enable == TD_FALSE) {
        return TD_SUCCESS;
    }
    return ss_mpi_ai3dnr_disable(vi_pipe);
}
