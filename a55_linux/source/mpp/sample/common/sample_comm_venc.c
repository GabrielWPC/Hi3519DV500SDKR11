...
extern int SetRtspVideo(unsigned int channel, unsigned char *pData,unsigned long length);
...
td_s32 sample_comm_venc_close_reencode(ot_venc_chn venc_chn)
{
    ...
    if (chn_attr.rc_attr.rc_mode == OT_VENC_RC_MODE_H264_ABR) {
		...
		rc_param.h264_vbr_param.min_qp = 10;
		rc_param.h264_vbr_param.min_i_qp =10;
		rc_param.h264_vbr_param.max_qp = 36;
		rc_param.h264_vbr_param.max_i_qp = 36;
    ...
    if (payload_type[index] == OT_PT_H265 && stream_proc_info->save_heif == TD_TRUE) {
		...
		if(index == 0)
		{
			...
			for (i = 0; i < stream->pack_cnt; i++) {
					...
					//(td_void)fflush(fd);
					//printf("stream->pack[i].len - stream->pack[i].offset =%d\n",stream->pack[i].len - stream->pack[i].offset);
					SetRtspVideo(index, stream->pack[i].addr + stream->pack[i].offset, stream->pack[i].len - stream->pack[i].offset);
				}
		}
#else
