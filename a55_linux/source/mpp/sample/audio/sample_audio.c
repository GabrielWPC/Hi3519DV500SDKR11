...
static td_void sample_audio_dimc_ai_ao_init_param(ot_aio_attr *ai_attr, ot_audio_dev *ai_dev,
    ot_aio_attr *ao_attr, ot_audio_dev *ao_dev)
{
...
static td_void sample_audio_dmic_ai_ao_inner(ot_audio_dev ai_dev, ot_ai_chn ai_chn,
    ot_audio_dev ao_dev, ot_ao_chn ao_chn)
{
    td_s32 ret;
...
td_s32 sample_audio_dmic_ai_ao(td_void)
{
    ...
    ret = sample_comm_audio_start_ai(ai_dev, ai_chn_cnt, &ai_attr, &ai_vqe_param, -1);
    if (ret != TD_SUCCESS) {
        sample_dbg(ret);
        goto dmic_err2;
    }
    ...
    ret = sample_comm_audio_start_ao(ao_dev, ao_chn_cnt, &ao_attr, g_in_sample_rate, g_aio_resample);
    if (ret != TD_SUCCESS) {
        sample_dbg(ret);
        goto dmic_err1;
    }
...
td_void sample_audio_usage(td_void)
{
    ...
    printf("\t    vqe_type: 0: none, 1: record, 2: talk, 3: talkv2 (default: 1)\n");
...
#endif
{
    ...
    if (index != 3 && argc != 2) { /* 2, 3:argv num */
        sample_audio_usage();
        return TD_FAILURE;
    }

    if (index == 3 && argc == 3) { /* 3:argv num */
