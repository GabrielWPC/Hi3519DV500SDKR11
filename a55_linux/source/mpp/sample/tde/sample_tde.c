...
static td_float min(td_float x, td_float y)
{
    return (x > y) ? y : x;
}
...
static td_void sample_tde_bit_blit(td_s32 handle, ot_tde_rect *src_rect, ot_tde_opt *opt, td_u32 next_on_show)
{
    ...
    for (i = 0; i < N_IMAGES; i++) {
        ...
        dst_rect.pos_x = (td_s32)(e_x_mid + r * cosf(ang) - g_img_sur[i].width / 2.0f); /* 2.0f alg data */
        dst_rect.pos_y = (td_s32)(e_y_mid + r * sinf(ang) - g_img_sur[i].height / 2.0f); /* 2.0f alg data */
...
static td_s32 sample_init_images_surface(td_void)
{
    td_u32 i;
...
static td_s32 sample_rotate_and_show(td_void)
{
    td_u32 times;
    struct fb_var_screeninfo var;
...
static td_s32 sample_tde_draw_graphic(td_void)
{
    td_s32 ret;
    /* 1. open tde device */
    ret = ss_tde_open();
...
static td_s32 sample_start_vo_by_intf_type(ot_vo_pub_attr *pub_attr)
{
    ...
    if (pub_attr->intf_type & OT_VO_INTF_BT1120) {
        ...
        if (sample_comm_vo_bt1120_start(pub_attr) != TD_SUCCESS) {
            return TD_FAILURE;
        }
    }
    return TD_SUCCESS;
...
int main(int argc, char *argv[])
{
    ...
    if (sample_comm_sys_vb_init(&vb_conf) != TD_SUCCESS) {
        return TD_FAILURE;
    }
