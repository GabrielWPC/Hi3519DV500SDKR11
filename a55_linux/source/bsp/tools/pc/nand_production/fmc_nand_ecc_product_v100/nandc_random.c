...
const unsigned int seed_table[SEED_COUNTS] = {
...
static unsigned int lfsr25(unsigned int seed)
{
	return (((seed >> 14) ^ (seed >> 17)) & 0xFF) | ((seed & 0x1FFFF) << 8); /* seed shift: 14 17 8 */
}

static int check_size_range(unsigned int pagesize, unsigned int oobsize)
{
	if (pagesize > UINT_MAX / 2 && oobsize > UINT_MAX / 2) /* 2: Calculate the half of UINT_MAX */
		return -1;
	else
		return 0;
}

void page_random_gen(unsigned char *pagebuf, enum page_type pagetype, enum ecc_type ecctype,
		unsigned int pageindex, unsigned int oobused)
{
	...
	if (check_size_range(pagesize, oobsize)) {
		printf("pagesize,oobsize out of range !\n");
		return;
	}
	...
	for (i = 0; i < pagesize + oobsize; i++) {
		...
		if ((pagesize == _16K) && (i >= (pagesize + oobsize) / 2)) { /* 2: next half of chip nand real page zise */
			if (seed_reset == 0) {
				seed = seed_table[pageindex];
				seed_reset = 1;
			}
			rnc8 = (((seed & 3) << 6) | ((seed >> 2) & 0x3f)) & 0xff; /* 3, 6, 2: random algorithm offset */
		} else {
			rnc8 = seed & 0xFF;
		}
